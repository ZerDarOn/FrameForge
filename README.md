...
- 本地 alpha 加权位移估计和相邻帧亮度变化检测；分析报告按项目恢复，迟到结果与进度不会串入其他项目。
- 配置 API Key 后可使用云端一致性检查及建议；这些调用不会在测试中自动执行。
- 全屏预览以及 PNG 序列、GIF、H.264 MP4 和无损动画 WebP 导出。
...
- 时间线支持多画格选择、批量偏移、视觉包围盒底边对齐，以及基于非透明像素包围盒的脚底建议；脚底建议会先展示移动数量和偏移范围，确认后才作为一条可撤销操作应用。跨帧脚部特征识别与基准点自动追踪仍未实现。
- 视频可按起止时间与抽帧 FPS 导入为 PNG 画格，长任务支持取消；MP4 导出使用 H.264 编码，动画 WebP 使用无损编码并保留透明通道，两者均可取消。运行桌面端前需安装 FFmpeg/ffprobe，并将其加入 PATH，或把 `FRAMEFORGE_FFMPEG_DIR` 指向二者所在目录。
