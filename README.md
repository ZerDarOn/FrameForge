...
- 生成任务支持 jobId/projectId 隔离、进度路由、协作式取消、失败重试和显式候选采用；失败/取消任务的不可变参数会按项目保存在本地，刷新后可恢复手动重试，成功或新任务会清除旧快照。已发出的供应商 HTTP 请求可能仍会执行到响应返回，取消会阻止迟到候选被前端接纳；运行中任务不会在刷新后自动续跑。
